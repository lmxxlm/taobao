...
export function waterFlows(parent, chirld){
    ...
    var num = Math.round(wscreenWidth/wchirldWidth[0].offsetWidth);//这是一个Math算法, 目的是将小数转变为整数,
    // 从而可以知道每行最多容纳几张图片
...
function getMinHeightOfCols(chirdArr, num){
    //创建数组, 用来盛放每一行的高度
    var onlyOneColsArr = [];
    var childWidth = chirdArr[0].offsetWidth;
    for(var i = 0; i<chirdArr.length; i++){
        var img = chirdArr[i].getElementsByTagName('img')[0];
        if(img.complete){

        	if(i<num){
	            //num为传进来的参数, 即为每行放图片的张数, 此步骤的目的是为了将第一行每张图片的高度遍历出来存放如新数组
	        	chirdArr[i].style.position = "absolute";
	            chirdArr[i].style.top = 0;
	            chirdArr[i].style.left = (childWidth) * i + 'px';
